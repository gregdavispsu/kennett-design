...
Version History
	...
	1.3.2 - 2015-01-05 - Chris Jean
		Fixed logic bug that prevented the "Time Parameters" query settings from modifying the query.
*/
...
class loopbuddy_queryitems {
	...
	public function get_wp_query( $query_id = -1 ) {
		...
		foreach ( $date_time_keys as $item_key => $item ) {
			if ( $keys[ $item ] != (string) intval( $keys[ $item ] ) ) {
				unset( $keys[ $item ] );
			}
		}
